...
class ResourcePriority:
    ...
    def set_allocation(self, priority: SystemPriority, percentage: int) -> bool:
        ...
        logger.info(
            f"Set {SystemPriority.get_name(priority)} allocation to {percentage}%"
        )
        return True
    ...
    def should_preempt(
        self, 
        current_priority: SystemPriority, 
        requesting_priority: SystemPriority
    ) -> bool:
        """
...
class StoragePriorityManager:
    ...
    def validate_allocation(
        self, 
        requested: Dict[SystemPriority, float]
        ...
        if total_requested > self.total_storage:
            return False, f"Total requested ({total_requested}GB) exceeds available ({self.total_storage}GB)"
